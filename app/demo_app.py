...
# Page config
st.set_page_config(
    page_title="AI Conversation Summarizer",
    page_icon="💬",
    layout="wide"
)
...
st.markdown("**Save 5+ hours per day** by automatically summarizing customer conversations, support tickets, and sales calls")

# Model status info
st.info("ℹ️ **Demo Mode:** Using base FLAN-T5 model (no training required). For production-quality results, fine-tune the model using this project's training scripts.")

# Add CTA buttons in header
col1, col2, col3 = st.columns([1, 1, 2])
with col1:
    st.link_button("📁 View Case Studies", "https://github.com/yourusername/flant5-dialogsum-sagemaker/blob/main/docs/FREELANCE_SHOWCASE.md")
with col2:
        ...
        if dialogue.strip():
            with st.spinner("Generating summary..."):
                try:
                    ...
                    with col3:
                        compression = round((1 - len(summary.split())/len(dialogue.split())) * 100)
                        st.metric("Compression", f"{compression}%")
                        
                except Exception as e:
                    st.error(f"Error generating summary: {str(e)}")
                    st.info("💡 **Note:** This demo requires the FLAN-T5 model to be downloaded (~1GB). On first run, it may take a few minutes to load.")
        else:
...
with tab2:
    ...
    with col1:
        num_agents = st.number_input("Number of team members", min_value=1, value=10, step=1)
        conversations_per_day = st.number_input("Conversations per person/day", min_value=1, value=20, step=1)
        minutes_per_conversation = st.number_input("Minutes to read/summarize each", min_value=1, value=3, step=1)
    
    with col2:
        hourly_rate = st.number_input("Average hourly rate ($)", min_value=10, value=30, step=5)
        time_saved_percent = st.slider("Time savings with AI (%)", min_value=50, max_value=90, value=80)
    ...
    st.success(f"💡 **ROI:** With typical implementation cost of $2,000-$5,000, you'll break even in less than {max(1, round(5000/monthly_savings))} month(s)!")
